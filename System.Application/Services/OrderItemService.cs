...
namespace System.Application.Services
{
    public class OrderItemService : IOrderItemService
    {
        ...
        public async Task AddOrderItemAsync(OrderItem orderItem)
        {
            ...
            // Check if the OrderItem already exists for this Order and MenuItem
            var existingItem = (await _unitOfWork.GetRepository<OrderItem, int>().FindAsync(oi => oi.OrderId == orderItem.OrderId && oi.MenuItemId == orderItem.MenuItemId && !oi.IsDeleted)).FirstOrDefault();
            if (existingItem != null)
                throw new CustomException("This menu item is already added to the order.", 400);
        ...
        {
            var orderItem = (await _unitOfWork.GetRepository<OrderItem, int>().FindAsync(oi => oi.OrderId == orderId && oi.MenuItemId == menuItemId && !oi.IsDeleted)).FirstOrDefault();
            if (orderItem == null)
